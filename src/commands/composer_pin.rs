...
fn write_toml_pin(path: &std::path::Path, version: &str) -> Result<PathBuf> {
    ...
    let table = composer
        .as_table_mut()
        .ok_or_else(|| eyre!("[composer] is not a table in {}", path.display()))?;
...
fn write_composer_pin(path: &std::path::Path, version: &str) -> Result<PathBuf> {
    ...
    let extra_obj = extra
        .as_object_mut()
        .ok_or_else(|| eyre!("`extra` is not an object"))?;
    let bougie = extra_obj
        .entry("bougie")
        .or_insert_with(|| serde_json::json!({}));
    let bougie_obj = bougie
        .as_object_mut()
        .ok_or_else(|| eyre!("`extra.bougie` is not an object"))?;
    let composer = bougie_obj
        .entry("composer")
        .or_insert_with(|| serde_json::json!({}));
    let composer_obj = composer
        .as_object_mut()
        .ok_or_else(|| eyre!("`extra.bougie.composer` is not an object"))?;
    composer_obj.insert("version".into(), serde_json::Value::String(version.into()));
