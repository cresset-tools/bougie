...
pub async fn provision(
    ...
    if let Some(existing_t) = existing.iter().find(|t| t.project == project) {
        ...
        let _ = ensure_host_block(paths, &hostname, project).await;
...
pub async fn deprovision(
    ...
    if tokio::fs::try_exists(&cfg).await.unwrap_or(false) {
        // `commands::server::config::remove_host` is sync TOML
        // editing; punt to the blocking pool to keep this fn pure
        // async without forcing the commands module to convert.
        let cfg_b = cfg.clone();
        let hostname_b = hostname.clone();
        let _ = tokio::task::spawn_blocking(move || {
            crate::commands::server::config::remove_host(&cfg_b, &hostname_b)
        })
        .await;
    }
    let _ = ping_reload_config(paths).await;
...
async fn ensure_host_block(paths: &Paths, hostname: &str, project: &Path) -> Result<()> {
    let cfg = server_toml_path(paths);
    let parent = cfg.parent().ok_or_else(|| eyre!("config path has no parent"))?;
    tokio::fs::create_dir_all(parent)
        .await
        .wrap_err_with(|| format!("creating {}", parent.display()))?;
    // `commands::server::config::add_host` is sync TOML editing;
    // run it on the blocking pool. Idempotent: returns `Ok(None)`
    // when the hostname is already present.
    let cfg_b = cfg.clone();
    let hostname_b = hostname.to_string();
    let project_b = project.to_path_buf();
    tokio::task::spawn_blocking(move || {
        crate::commands::server::config::add_host(&cfg_b, &hostname_b, &project_b, None)
    })
    .await
    .map_err(|e| eyre!("add_host task panicked: {e}"))?
...
async fn ping_reload_config(_paths: &Paths) -> Result<()> {
    ...
    let mut stream = tokio::time::timeout(CONTROL_TIMEOUT, UnixStream::connect(&sock))
        .await
        .map_err(|_| eyre!("connecting to {} timed out", sock.display()))?
        .wrap_err_with(|| format!("connecting to {}", sock.display()))?;
    tokio::time::timeout(
        CONTROL_TIMEOUT,
        stream.write_all(b"{\"v\":1,\"method\":\"reload-config\"}\n"),
    )
    .await
    .map_err(|_| eyre!("writing reload-config timed out"))?
    ...
    let _ = tokio::time::timeout(CONTROL_TIMEOUT, stream.read_to_string(&mut reply)).await;
