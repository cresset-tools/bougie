...
use bougie_semver::version::{CmpOp, Version};
use bougie_semver::Stability;
                ...
                match (Version::parse(v), Constraint::parse(c)) {
                    (Ok(version), Ok(constraint)) => {
                        if !constraint.matches(&version) {
                            failures.push(format!(
                                "{id}: {v:?} should satisfy {c:?} but doesn't"
                            ));
                        }
                    }
                    (Err(e), _) => failures.push(format!(
                        "{id}: {v:?} {c:?} version parse err: {e}"
                    )),
                    (_, Err(e)) => failures.push(format!(
                        "{id}: {v:?} {c:?} constraint parse err: {e}"
                    )),
                }
                *covered += 1;
            }
            ...
            for case in &suite.cases {
                ...
                match (Version::parse(v), Constraint::parse(c)) {
                    (Ok(version), Ok(constraint)) => {
                        if constraint.matches(&version) {
                            failures.push(format!(
                                "{id}: {v:?} should NOT satisfy {c:?} but does"
                            ));
                        }
                    }
                    (Err(e), _) => failures.push(format!(
...
fn sort_versions(input: &[String], ascending: bool) -> Option<Vec<String>> {
    ...
    parsed.sort_by(|a, b| if ascending { a.1.cmp(&b.1) } else { b.1.cmp(&a.1) });
