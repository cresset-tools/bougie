...
/// Optional hook for injecting `BOUGIE_SERVICE_*` env vars into
/// recipe shell steps. Set once at process startup by the bougie
/// binary (which owns the IPC client to bougied); leaving it unset
/// keeps the recipe engine usable in isolation.
