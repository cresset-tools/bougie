...
/// The fragment [`Paths::ensure_cli_defaults`] materializes. CLI-only:
/// Magento's `bin/magento` and Composer routinely blow past the
/// engine's 128M default, and CLI PHP on common distros ships with
/// `memory_limit = -1`; a project or tool conf.d fragment (or
/// `ini_set()`) overrides it. FPM is never affected.
pub const CLI_DEFAULTS_INI: &str = "\
; Generated by bougie. CLI-only memory-limit lift; a project or tool
; conf.d fragment or ini_set() overrides it. FPM is never affected
; (server pools assemble their own scan dir).
memory_limit = -1
";
...
impl Paths {
    ...
    /// layered into `PHP_INI_SCAN_DIR` for **CLI** PHP execution:
    /// `bougie run`, the `php`/`composer` argv0 shims, script runs,
    /// and tool runs (ordered so project/tool fragments still win).
    /// Holds the CLI memory-limit lift; because the scan dir travels
    /// via the environment, child PHP processes (`PHP_BINARY` via
    /// Symfony Process, `bin/magento`, …) inherit it too — which
    /// argv-level `-d` overrides can't do. The server's FPM pools
    /// assemble their own scan dir and never include this layer.
    pub fn cli_defaults(&self) -> PathBuf {
        self.local.join("cli-defaults")
    }

    /// Ensure `$BOUGIE_LOCAL/cli-defaults/99-memory-limit.ini` exists
    /// with the expected content, returning the directory to layer
    /// into `PHP_INI_SCAN_DIR`. Idempotent (content-compared,
    /// rewritten only on drift) and best-effort: any failure returns
    /// `None` and the caller proceeds without the lift.
    pub fn ensure_cli_defaults(&self) -> Option<PathBuf> {
        let dir = self.cli_defaults();
        let ini = dir.join(CLI_DEFAULTS_INI_NAME);
        let up_to_date =
            std::fs::read_to_string(&ini).is_ok_and(|current| current == CLI_DEFAULTS_INI);
        if !up_to_date {
