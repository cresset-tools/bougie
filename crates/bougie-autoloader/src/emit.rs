...
use crate::collect::{Entry, FileEntry};
...
pub(crate) fn psr4(entries: &[Entry]) -> String {
    let mut out = String::with_capacity(PSR4_HEADER.len() + entries.len() * 64 + ARRAY_FOOTER.len());
...
pub(crate) fn psr0(entries: &[Entry]) -> String {
    let mut out = String::with_capacity(PSR0_HEADER.len() + entries.len() * 64 + ARRAY_FOOTER.len());
...
pub(crate) fn files(entries: &[FileEntry]) -> String {
    let mut out = String::with_capacity(FILES_HEADER.len() + entries.len() * 80 + ARRAY_FOOTER.len());
