...
pub fn prepare(
    ...
    if let Some(defaults) = ensure_cli_defaults(paths) {
        scan.push(&defaults);
        scan.push(SCAN_DIR_SEP);
    }
...
/// Bougie's CLI ini defaults, layered between the install's conf.d
/// and the tool's own conf.d. Currently just the memory-limit lift:
/// Magento's `bin/magento` and Composer routinely blow past php.ini's
/// 128M default, and `memory_limit=-1` is Magento's own CLI
/// recommendation — the same stance the project-side `php` argv[0]
/// shim takes with `-d memory_limit=-1`. An ini fragment (not `-d`)
/// because `PHP_INI_SCAN_DIR` is inherited by child PHP processes the
/// tool spawns (e.g. n98-magerun2 running `PHP_BINARY bin/magento`),
/// which argv flags never reach.
const CLI_DEFAULTS_INI: &str = "\
; Generated by bougie tool-exec. CLI-only memory-limit lift for tool
; runs; a tool conf.d fragment or ini_set() overrides it. FPM is never
; affected (tools exec the CLI binary).
memory_limit = -1
";

/// Name sorts after the install's `00-*`/`NN-<ext>.ini` fragments;
/// ordering across scan dirs is by dir position anyway.
const CLI_DEFAULTS_INI_NAME: &str = "99-memory-limit.ini";

/// Ensure `$BOUGIE_LOCAL/cli-defaults/99-memory-limit.ini` exists with
/// the expected content, returning the directory to layer into
/// `PHP_INI_SCAN_DIR`. Idempotent (content-compared, rewritten only on
/// drift) and best-effort: any failure returns `None` and the tool
/// runs without the lift — same failure posture as the unzip shim.
fn ensure_cli_defaults(paths: &Paths) -> Option<PathBuf> {
    let dir = paths.cli_defaults();
    let ini = dir.join(CLI_DEFAULTS_INI_NAME);
    let up_to_date = std::fs::read_to_string(&ini)
        .is_ok_and(|current| current == CLI_DEFAULTS_INI);
    if !up_to_date {
        std::fs::create_dir_all(&dir).ok()?;
        std::fs::write(&ini, CLI_DEFAULTS_INI).ok()?;
    }
    Some(dir)
}
...
mod tests {
    ...
    fn cli_defaults_ini_written_and_healed() {
        ...
        let dir = ensure_cli_defaults(&paths).expect("cli-defaults must materialise");
        let ini = dir.join(CLI_DEFAULTS_INI_NAME);
        ...
        ensure_cli_defaults(&paths).unwrap();
        assert_eq!(std::fs::read_to_string(&ini).unwrap(), CLI_DEFAULTS_INI);
    }
