...
pub fn plan_closure_bytes(paths: &Paths, manifest: &Manifest, bar: &DownloadBar) {
    for closure in &manifest.closure {
        let store_path =
            store_dir_for_closure(paths, &closure.name, &closure.version, &closure.hash);
        if !store_path.exists() {
...
mod tests {
    ...
    fn tool_manifest_with_closure(entries: &[(&str, &str, &str)]) -> bougie_index::wire::Manifest {
    ...
    fn plan_closure_bytes_only_counts_missing() {
        ...
        let manifest: bougie_index::wire::Manifest = serde_json::from_value(serde_json::json!({
