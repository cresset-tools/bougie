...
fn write_fragment_into(
    ...
    let mut body =
        format!("; managed by bougie — do not edit; regenerated by `bougie ext add {name}`\n",);
...
fn has_numeric_prefix(filename: &str) -> bool {
    let Some(dash) = filename.find('-') else {
        return false;
    };
...
mod tests {
    ...
    fn writes_zend_extension_fragment() {
        ...
        let path = write_ext_fragment(td.path(), "xdebug", &so, LoadDirective::ZendExtension, &[])
            .unwrap();
        let body = std::fs::read_to_string(&path).unwrap();
    ...
    fn pdo_drivers_use_35_prefix() {
        ...
        let path =
            write_ext_fragment(td.path(), "pdo_mysql", &so, LoadDirective::Extension, &[]).unwrap();
    ...
    fn db_drivers_use_40_prefix() {
        ...
        for name in ["mysqli", "sqlite3", "pgsql"] {
            ...
            let path =
                write_ext_fragment(td.path(), name, &so, LoadDirective::Extension, &[]).unwrap();
            assert!(
    ...
    fn xdebug_fragment_includes_mode_and_trigger() {
        ...
        assert!(
            body.contains("xdebug.start_with_request=trigger"),
            "got: {body}"
        );
    }
    ...
    fn write_ext_fragment_emits_path_extra_markers() {
        ...
        let path = write_ext_fragment(td.path(), "imagick", &so, LoadDirective::Extension, &extras)
            .unwrap();
        let body = std::fs::read_to_string(&path).unwrap();
        assert!(
    ...
    fn php_ini_scan_dir_layers_cli_defaults_before_conf_d() {
        ...
        let ini = paths
            .cli_defaults()
            .join(bougie_paths::CLI_DEFAULTS_INI_NAME);
        assert!(
            std::fs::read_to_string(ini)
                .unwrap()
                .contains("memory_limit = -1"),
